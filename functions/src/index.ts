...
const SYNAPSE_SERVER_SECRET = defineSecret("SYNAPSE_SERVER_SECRET");
...
export const steamLogin = https.onRequest({ secrets: [STEAM_API_KEY] }, async (req, res) => {
  if (req.method === "OPTIONS") { res.set(cors).status(204).send(""); return; }
  if (req.method !== "POST")    { res.set(cors).status(405).json({ error: "Method not allowed" }); return; }

  const { openidParams } = req.body as { openidParams?: Record<string, string> };
  if (!openidParams) { res.set(cors).status(400).json({ error: "openidParams ausente." }); return; }
  ...
  if (!steamId) { res.set(cors).status(400).json({ error: "SteamID não encontrado." }); return; }
  ...
  if (apiKey) {
    try {
      ...
      const profile = await profileRes.json() as { response: { players: Array<{ personaname: string; avatarfull: string }> } };
      const player = profile.response?.players?.[0];
      if (player) { nickname = player.personaname; avatarUrl = player.avatarfull; }
    } catch { /* continua sem avatar */ }
  }

  // Upsert no Firestore
  await db.collection("players").doc(steamId).set(
    { nickname, steam_id: steamId, avatar_url: avatarUrl, updated_at: admin.firestore.FieldValue.serverTimestamp() },
    { merge: true }
  );
...
export const faceitStats = https.onRequest(async (req, res) => {
  if (req.method === "OPTIONS") { res.set(cors).status(204).send(""); return; }

  const { nickname } = req.body as { nickname?: string };
  if (!nickname) { res.set(cors).status(400).json({ error: "Informe o nickname Faceit." }); return; }

  const apiKey = functions.config().faceit?.api_key ?? process.env.FACEIT_API_KEY ?? "";
  if (!apiKey) { res.set(cors).status(500).json({ error: "FACEIT_API_KEY não configurada." }); return; }
  ...
  if (!playerRes.ok) { res.set(cors).status(502).json({ error: "Jogador Faceit não encontrado." }); return; }
  ...
  let statsRes = await fetch(
    `https://open.faceit.com/data/v4/players/${playerData.player_id}/stats/${gameId}`,
    { headers: { Authorization: `Bearer ${apiKey}` } }
  );
  if (!statsRes.ok) {
    gameId = "csgo";
  ...
  if (!statsRes.ok) { res.set(cors).status(502).json({ error: "Stats Faceit indisponíveis." }); return; }

  const statsData = await statsRes.json() as {
    lifetime: Record<string, unknown>;
    segments: Array<{ type: string; label: string; stats: Record<string, unknown> }>;
  };
...
export const faceitHistory = https.onRequest(async (req, res) => {
  if (req.method === "OPTIONS") { res.set(cors).status(204).send(""); return; }

  const { playerId, game = "cs2", limit = 20 } = req.body as { playerId?: string; game?: string; limit?: number };
  if (!playerId) { res.set(cors).status(400).json({ error: "playerId ausente." }); return; }

  const apiKey = functions.config().faceit?.api_key ?? process.env.FACEIT_API_KEY ?? "";
  const histRes = await fetch(
    `https://open.faceit.com/data/v4/players/${playerId}/history?game=${game}&limit=${limit}`,
    { headers: { Authorization: `Bearer ${apiKey}` } }
  );
  if (!histRes.ok) { res.set(cors).status(502).json({ error: "Histórico indisponível." }); return; }
...
export const syncLevels = https.onRequest(async (req, res) => {
  if (req.method === "OPTIONS") { res.set(cors).status(204).send(""); return; }

  const { steamId, nickname } = req.body as { steamId?: string; nickname?: string };
  if (!steamId || !nickname) {
  ...
  await db.collection("players").doc(steamId).set(
    { faceit_level: level, faceit_elo: elo, faceit_nickname: nickname },
    { merge: true }
  );
...
export const receiveMatchStats = https.onRequest({ secrets: [SYNAPSE_SERVER_SECRET] }, async (req, res) => {
  if (req.method === "OPTIONS") { res.set(cors).status(204).send(""); return; }
  if (req.method !== "POST")   { res.set(cors).status(405).send(""); return; }

  const secret = SYNAPSE_SERVER_SECRET.value();
  const incomingSecret = req.headers["x-synapse-secret"] as string | undefined;
  ...
  const teamA: string[] = mixData.team_a ?? [];
  const teamB: string[] = mixData.team_b ?? [];
  ...
  for (const p of body.players) {
    ...
    if (score > mvpScore) { mvpScore = score; mvpId = p.steam_id; }
  ...
  if (body.event === "match_end") {
    ...
    const scoreA: number = mixFinal.score_a ?? 0;
    const scoreB: number = mixFinal.score_b ?? 0;
...
export const updateMixScore = https.onRequest(async (req, res) => {
  if (req.method === "OPTIONS") { res.set(cors).status(204).send(""); return; }

  const { mix_id, score_a, score_b } = req.body as { mix_id?: string; score_a?: number; score_b?: number };
  if (!mix_id) { res.set(cors).status(400).json({ error: "mix_id ausente." }); return; }
